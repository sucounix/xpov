...
data "aws_iam_policy_document" "kms_resource_policy_doc" {
  statement {
    ...
    actions   = local.kms_write_actions_sm
    resources = ["*"]

    condition {
      test     = "StringNotLike"
      variable = "aws:userId"
      values   = local.saml_user_ids_sm
    }
  }
  ...
  statement {
    ...
    actions   = local.kms_read_actions_sm
    resources = ["*"]

    condition {
      test     = "StringNotLike"
      variable = "aws:userId"
      values   = local.role_and_saml_ids_sm
    }
  }
...
variable "secrets_saml_users_sm" {
  type = list(string)
}
